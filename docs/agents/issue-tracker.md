...
- **Child ticket**: an issue linked to the map as a GitHub sub-issue using `gh api`. Where sub-issues are unavailable, add the child to a task list in the map body and put `Part of #<map>` at the top of the child body. Use a `wayfinder:<type>` label: `research`, `prototype`, `grilling`, or `task`.
- **Blocking**: use GitHub’s native issue dependencies. Add an edge with `gh api --method POST repos/<owner>/<repo>/issues/<child>/dependencies/blocked_by -F issue_id=<blocker-db-id>`, where `<blocker-db-id>` is the blocker’s numeric database ID from `gh api repos/<owner>/<repo>/issues/<n> --jq .id`.
- **Frontier query**: list the map’s open children with `gh issue list --state open`, discard assigned or blocked tickets, and select the first remaining ticket in map order.
